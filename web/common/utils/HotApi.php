...
class HotApi {
    ...
    function get_account_balance() {
        $this->api_method = "/v1/account/accounts/".$this->account."/balance";
        $this->req_method = 'GET';
        $url = $this->create_sign_url(array());
        return json_decode($this->curl($url),1);
    }
    ...
    // 获取账户余额
    function get_balance($account_id=NULL) {
        $account_id = $account_id ? $account_id : $this->account;
    ...
    function create_sign_url($append_param = array()) {
        ...
        return $this->api_url.$this->api_method.'?'.$this->bind_param($param);
    }
    // 组合参数
    ...
    function curl($url,$postdata = array()) {
         try {
            ...
            if (strtoupper($this->req_method) == 'POST' && !is_null($postdata)) {
