...
//        $data['total_token_bonus'] = $total_token_bonus; // P3D奖励总额
    ...
    public function buy() {
        if (IS_POST) {
            ...
            try {
                ...
                $balanceM->startTrans();
                //扣除用户余额
                $total_amount = $token_total_price + $token_total_price * $token_trading_rate / 100;
                ...
                $change_type = 0; //减少
                $remark = '购买token';
                $r_id = $recordM->addRecord($this->user_id, $coin_id, $total_amount, $before_amount, $after_amount, $type, $change_type, '', '', '', $remark, 0, 0);
                if (!$r_id) {
                    $balanceM->rollback();
    ...
    public function sale(){
        if(IS_POST){
            ...
            if(empty($user_token) || $user_token['token'] < $p3d_num){
                return $this->failJSON('您的令牌数量不足');
            }
            //计算总价 递减
            $confM = new \addons\fomo\model\TokenConf();
            $token_float = $confM->getValByName('token_float');//token 浮动值
            ...
            try{
                $token_trading_rate = $confM->getValByName('sale_tax'); //token 扣除百分比
                $total_amount = $token_total_price - $token_total_price * $token_trading_rate / 100;
                $coinM = new \addons\config\model\Coins();
                $coin = $coinM->getCoinByName();//获取eth id
                $coin_id = $coin['id'];
                ...
                $sequeueM = new \addons\fomo\model\BonusSequeue();
                if ($token_trading_rate > 0) {
                    $buy_assign = $confM->getValByName('sale_assign'); //买入手续费分配 0:令牌持有者 1:平台
                    if($buy_assign == 0){
                        $p3d_amount = $this->countRate($token_total_price, $token_trading_rate); //发放的分红
                        $p3d_amount = $this->_sendToSelf($this->user_id, $coin_id, $p3d_amount);
                        $sequeueM->addSequeue($this->user_id, $coin_id, $p3d_amount, 0, 0);
