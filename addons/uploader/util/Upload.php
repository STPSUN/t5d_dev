...
namespace addons\uploader\util;
...
class Upload {
    ...
    private $config = array(
        ...
        'subName' => array('GetMD5Name', '32'), //子目录创建方式，[0]-函数名，[1]-参数，多个参数使用数组
        'rootPath' => './uploads/', //保存根路径
        'savePath' => '', //保存路径
        'saveName' => array('GetMD5Name', '32'), //上传文件命名规则，[0]-函数名，[1]-参数，多个参数使用数组
    ...
    public function upload($files = '') {
        ...
        foreach ($files as $key => $file) {
            ...
            $orig_name = $file['name']; //lcj 加入原始文件名
            if (!isset($file['key']))
                $file['key'] = $key;
    ...
    private function setDriver($driver = null, $config = null) {
        ...
        $class = strpos($driver, '\\') ? $driver : 'addons\\uploader\\util\\driver\\' . ucfirst(strtolower($driver));
    ...
    private function checkSize($size) {
        if (\think\App::$debug){
            \think\log::write('文件大小:'.$size);
        }
        return (0 == $this->maxSize) || !($size > $this->maxSize);
    }
    ...
    private function getName($rule, $filename) {
        ...
        if (is_array($rule)) { //数组规则
            ...
            $name = call_user_func_array($func, array('length'=>$param[0],'org_name'=>$filename));
