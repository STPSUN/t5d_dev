...
class Template
{
    ...
    public function fetch($template, $vars = [], $config = [])
    {
        ...
        if ($template) {
             //2017-09-08 lcj修改
            $template_name = $template . Request::instance()->isPjax();
            $cacheFile = $this->config['cache_path'] . $this->config['cache_prefix'] . md5($template_name) . '.' . ltrim($this->config['cache_suffix'], '.');
            ...
            if (preg_match($regex, $template, $matches)) {
                if (!isset($array[$matches['name']])) {
                    ...
                    if (!Request::instance()->isPjax()) {
                        ...
                        $extend = str_replace($matches[0], '', $template);
                        $baseBlocks = array_merge($blocks, $this->parseBlock($extend));
                    }
                    return;                    
                }
            } else {
